      ...
      if (!isMobile.value && sectionRef.value) {
        ...
        if (st) {
          st.eventCallback('onLeave', () => {
            ...
            gsap.set('.video-box', { clipPath: 'circle(150% at 50% 50%)', WebkitClipPath: 'circle(150% at 50% 50%)' })
            st.kill(true)
            ScrollTrigger.refresh()
          })
        }
...
<template>
  <section class="vd-pin-section" ref="sectionRef">
    <div
      class="video-box"
      :style="{
        clipPath: isMobile ? 'circle(150% at 50% 50%)' : 'circle(8% at 50% 50%)',
        WebkitClipPath: isMobile ? 'circle(150% at 50% 50%)' : 'circle(8% at 50% 50%)',
      }"
    >
      <video
    ...
    <div class="overlay-center md:scale-100">
      <img src="/images/circle-text.svg" alt="Circle text" class="spin-circle" />
      <div class="play-btn">
